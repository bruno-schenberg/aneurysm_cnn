...
### What to Expect
1. **Console Output**: You will see a stream of standard python `logging` output detailing which exams are being discovered, processed, skipped (due to idempotency), or failed (due to corruption).
...
4. **Output Data**: NIfTI (`.nii.gz`) files will be safely written to the specified output directory (e.g., `/mnt/data/cases-3/nifti`), organized by class, without ever altering the original raw DICOM files.