...
def join_class_data(validated_data, classes_csv_path):
    ...
    It matches 'fixed_name' from the validated data with 'exame' from the CSV.
    ...
    Returns:
        list: The validated_data list, with 'classe' and 'location' added.
    """
    ...
    try:
        with open(classes_csv_path, 'r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                exam_name = row.get('exame', '').strip().upper()
                if exam_name:
                    class_map[exam_name] = {
